...
function HomeCont() {
  ...
  //set movie page func 
  function openMovie(title, year, image, cover, descript, rating, genre, runtime, starring, director) {
    setTitle(title)
    setYear(year)
    setImage(image)
    setCover(cover)
  ...
  return (  
    <div className="homecont">  
      ...
      <MoviePage title={title} year={year} image={image} cover={cover} descript={descript} rating={rating} genre={genre} runtime={runtime} starring={starring} director={director}/>
    </div> 
  ) 
} 
